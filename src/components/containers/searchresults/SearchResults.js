...
import EpisodeDescription from '../../EpisodeDesc'
    ...
    <>
      <div className="flex md:flex-row flex-col md:space-x-4">
        <div className="md:w-2/5 w-full p-4">
          <h1 className="text-left text-gray-100 text-2xl font-bold pb-2">
            Top result
          </h1>
          <div
            className={` bg-gradient-to-b ${getGenreGradientColor(
              topResult.genres[0],
            )} bg-gray-900 p-5 rounded-lg hover:bg-gray-800 cursor-pointer`}
            onClick={() => handleClick(topResult.collectionId)}
          >
            <img
            ...
            <div className="pt-5 text-gray-300 text-left">
              ...
              <div className="text-left pt-4">
                {topResult.genres.map((genre) => (
                  <span
                    className={`text-xs text-white p-0.5 mr-1 rounded ${getGenreColor(
                      genre,
                    )}`}
                    key={genre}
                  >
        ...
        <div className="md:w-3/5 w-full p-4">
          ...
          {episodes &&
            episodes.map((item) => (
              <div
                key={item.trackId}
                className="flex flex-row bg-gray-900 mb-2 hover:bg-gray-800"
              >
                <img
      ...
      <h1 className="text-left text-gray-100 text-2xl font-bold pb-5">
        Podcast that match '{activeSearchText}'
      </h1>
      <div className="flex flex-wrap flex-row">
        {otherPodcasts.map((podcast) => (
          <div
            className="xl:w-1/5 md:w-1/3 sm:w-1/3 w-1/3 p-1"
          ...
          >
            <div onClick={() => handleClick(podcast.collectionId)}>
              <div className="p-3 bg-gray-900 hover:bg-gray-800 cursor-pointer rounded-lg">
                <img
                  className="rounded-lg w-full object-contain mb-1"
