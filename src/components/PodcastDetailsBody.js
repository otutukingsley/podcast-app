...
import LoadMore from './containers/spinner/Loading'
...
const PodcastDetailsBody = ({ episodes, handlePause, handlePlay }) => {
  ...
  const getMoreData = () => {
    ...
    setTimeout(() => {
      setCurrent(
        current.concat(episodes.slice(count.prev + 10, count.next + 10)),
      )
    }, 1000)
    setCount((prevState) => ({
    ...
    <div className="flex max-w-full md:px-8 px-0 text-left text-gray-300">
      ...
      <div className="w-5/12 h-32 pl-12 md:block hidden">
        ...
        <p className="text-1xl font-thin pt-4">
          ...
          corrupti cupiditate quibusdam magnam ipsum. Eveniet numquam sit,
          dignissimos rerum nihil quod mollitia, natus tempora ullam, esse
          dolores error repellat nisi.
        </p>
      </div>
    </div>
  )
