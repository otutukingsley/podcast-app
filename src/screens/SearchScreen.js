...
import SearchResultContainer from '../components/containers/searchresults/SearchResultsContainer'
import SearchTopGenres from '../components/containers/searchtopgenres/SearchTopGenres'
...
function SearchScreen(props) {
  ...
  const {
    resultCount: podcastResultCount,
...
const getPodcasts = async (text) => {
  const response = await axios.get(
    `${BASE_URL}search?term=${text}&entity=podcast`,
  )
...
const getEpisodes = async (text) => {
  const response = await axios.get(
    `${BASE_URL}search?term=${text}&entity=podcastEpisode&limit=4`,
  )
  return response.data
}
