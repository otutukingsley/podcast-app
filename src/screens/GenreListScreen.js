      ...
      {results ? (
        <>
          <section className="container px-5 mx-auto">
            <h1 className="text-left text-gray-100 text-2xl py-2 sm:pt-10 font-bold pb-5 ">
              {results[0].genres[0]}
            </h1>

            <div className="flex flex-wrap">
              {results.map((podcast) => (
                <div className="xl:w-1/5 md:w-1/2" key={podcast.collectionName}>
                  <div className="p-1">
                    <div
                      className="p-3 bg-gray-900 hover:bg-gray-800 cursor-pointer rounded-lg"
...
const getPodcasts = async (genreId) => {
  const response = await axios.get(
    `${BASE_URL}search?term=podcast&genreId=${genreId}&limit=200`,
  )
